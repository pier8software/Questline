...
1. **Request** - Add a record inside the relevant `Messages/Requests.cs`, annotate with `[Verbs("...")]`, implement `IRequest` (including the `static abstract CreateRequest` factory):
   ```csharp
   [Verbs("eat")]
   ...
   ```
2. **Response** - Add a record inside `Messages/Responses.cs` implementing `IResponse`. The response formats its own `Message` property:
   ```csharp
   ...
   ```
3. **Handler** - Create a handler class in `Handlers/` implementing `IRequestHandler<T>`:
   ```csharp
